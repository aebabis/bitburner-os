import { getTailModal } from '../lib/modal.js';
import { getGoalSnapshot } from '../lib/goal-tracker.js';
...
const PALETTE = [
  ...
  '#56b6c2', '#d19a66', '#7cc499', '#ff9f7f', '#da70d6',
];
...
export async function main(ns) {
  ...
  zoomIn.textContent = '+';
  zoomOut.textContent = '−';
  zoomIn.addEventListener('click', () => { viewMs = Math.max(30_000, viewMs / 2); });
  zoomOut.addEventListener('click', () => { viewMs = Math.min(30 * 60_000, viewMs * 2); });
  ...
  const render = () => {
    ...
    for (const faction of topFactions) {
      ...
      for (const snap of drawSource) {
        ...
        if (!plan) { started = false; continue; }
        const x = toX(snap.ts);
    ...
    // Stats footer
    const lastSnap = visible.length > 0 ? visible[visible.length - 1] : snapshots[snapshots.length - 1];
