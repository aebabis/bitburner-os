...
export const getAllServices = (ns: NS, player: (_ns: NS) => Player) => {
  ...
  return [
    ...
    AnyHostService(ns, () => !useWolf(), couldTrade)('/bin/broker/broker.ts'),
    AnyHostService(ns, bladesAvailable, useBlade)('/bin/blades/blades.ts'),
    AnyHostService(ns, () => hacknetAvailable && playerLikesHacknet)('/bin/hacknet.ts'),
    AnyHostService(ns, () => gangsAvailable, inCriminalFaction)('/bin/gang/mob-boss.ts'),
    AnyHostService(ns, () => corpAvailable, corpReady)('/bin/corporation/corporation.ts'),
    ...
    AnyHostService(ns, () => !hasSingularity)('/bin/hinter.ts'),
    AnyHostService(ns, () => !hasSingularity)('/bin/trailblazer.ts'),
