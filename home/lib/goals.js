 ...
 * @returns {Goal}
 */
export const goal = (type, desc, isDone, requirement = undefined) => ({
  type,
  desc,
  isDone,
  requirement,
...
export const getGoals = (ns) => {
  ...
  const combatReq = /** @type {Record<string,number>} */ (COMBAT_REQUIREMENTS)[targetFaction];
  const hackReq = /** @type {Record<string,number>} */ (HACKING_REQUIREMENTS)[targetFaction];
  ...
  goals.push(
    goal("FACTION_JOIN", "Join " + targetFaction, () => factions.includes(targetFaction)),
    goal("FACTION_REP", "Gain " + repNeeded + " rep",
      () => repNeeded != null && factionRep[targetFaction] >= repNeeded),
