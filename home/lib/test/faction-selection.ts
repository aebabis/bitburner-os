...
// This suite tests the selection algorithm against hand-computed synthetic aug pools, not
// formula fidelity (see test/formulas.ts for that) -- it deliberately stays on getMockFormulas
// rather than real ns.formulas so the hand-verified utility math in the comments below holds.

// Unlike buildPerson's own all-1s default, every stat here starts at 0: this file's tests are
// deliberately isolated to a single skill's effect for hand-verifiable math, and buildPerson's
// production-oriented defaults would introduce an unintended (if small) intelligence/combat
// contribution into that math.
const ZERO_SKILLS: Skills = {
  hacking: 0,
  strength: 0,
  defense: 0,
  dexterity: 0,
...
// Explicit Person fixture (full .mults, not an absent field the mock happens to default via
// `?? 1`) so these tests don't depend on getMockFormulas's optional-chaining being forgiving.
// augs: [] keeps mults neutral regardless of which staticData a given test is using.
const mockPlayer = (skills: Partial<Skills> = {}, extra: Record<string, unknown> = {}) => ({
  ...buildPerson({} as StaticData, { augs: [] }),
  skills: { ...ZERO_SKILLS, ...skills },
  ...extra,
});
...
export async function main(ns: NS) {
  ...
  describe('faction selection', () => {
    ...
    it('Netburner augs owned → CyberSec becomes best', () => {
      const data = makeStaticData({
        Netburners: [NB_A, NB_B, NB_C],
        CyberSec: [CS_A, CS_B],
      });
    ...
    it('Netburners + CyberSec owned → city faction with hacking augs wins', () => {
      const data = makeStaticData({
        Netburners: [NB_A, NB_B, NB_C],
        CyberSec: [CS_A, CS_B],
    ...
    it('endgame faction unlocks once numAugmentations threshold is met', () => {
      const data = makeStaticData(
        {
          Netburners: [NB_A],
    ...
    it('returns null faction when no accessible faction has any augs', () => {
      const data = makeStaticData({});
    ...
    it('faction with significant current rep is preferred over higher raw utility', () => {
    ...
    it('select The Syndicate over The Covenant when combat augs are still needed', () => {
      const ownedAugs = [
      ...
      const player = mockPlayer(
        { hacking: 500, strength: 500, defense: 500, dexterity: 500, agility: 500 },
        { factions: [] },
      );
