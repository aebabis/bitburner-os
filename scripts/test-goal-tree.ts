...
describe('buildFactionGoalTree', () => {
  ...
  it('utility is higher for a faster plan (same value, shorter eta)', () => {
    const fast = buildFactionGoalTree(mockNs, 'F' as FactionName, valueTestData(2)); // eta=50s
    const slow = buildFactionGoalTree(mockNs, 'F' as FactionName, valueTestData(1)); // eta=100s
    assert.ok(fast);
    assert.ok(slow);
...
describe('getMockFormulas reputation', () => {
  ...
  it('donationForRep scales with faction_rep aug mult', () => {
    const base = getMockFormulas({ installedAugmentations: [] } as any);
    const boosted = getMockFormulas({
      installedAugmentations: ['FactionAug'],
      augmentationStats: { FactionAug: { faction_rep: 1.25 } },
    } as any);
    const baseCost = base.reputation.donationForRep(1000, {} as Person);
    const boostedCost = boosted.reputation.donationForRep(1000, {} as Person);
